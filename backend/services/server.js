...
const addStoryToDB = async (storyID, story) => {
  try {
    // Add the story to the Stories table
    const addStoryParams = {
      TableName: "Stories", // The name of your Stories table
      Item: {
        StoryID: storyID,           // The StoryID value you want to use
        DateCreated: new Date().toISOString(), // Timestamp for DateCreated (or your custom date)
        Content: story            // The content for the story
      }
    };
    await dynamoDB.put(addStoryParams).promise();
  ...
  try {
    // For Players with PlayerID 1 and 2, update their records
    const playerIDs = [1, 2];
    for (const playerID of playerIDs) {
      const updateParams = {
        TableName: "Players",
        Key: { PlayerID: playerID },
        UpdateExpression: "ADD StoriesParticipated :storyID",
        ExpressionAttributeValues: {
          ":storyID": dynamoDB.createSet([storyID.trim()])
        }
      };

      // Update DynamoDB for each player
      try {
        await dynamoDB.update(updateParams).promise();
        console.log(`Updated Players Table for PlayerID ${playerID}`);
      } catch (err) {
        console.error(`Error updating Players Table for PlayerID ${playerID}:`, err);
      }
    }
  } catch (err) {
    console.error("Error updating Players Table:", err);
  }
};
...
io.on("connection", (socket) => {
  ...
  socket.on("create_room", (callback) => {
    ...
    const storyID = cryptoRandomString({ length: 12, type: 'alphanumeric' });
    rooms[room] = {
      users: [],
      creator: socket.id,
      gameStarted: false,
